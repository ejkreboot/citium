<script lang="ts">
	...
	import { addDays, addMonths, WEEKDAYS_LONG, MONTHS, dayKey } from '$lib/date';
	...
	const initialView = (['year', 'month', 'day'].includes(page.url.searchParams.get('v') ?? '')
...
<header class="head">
	...
	<div class="controls">
		<div class="segmented">
			<button aria-pressed={view === 'year'} onclick={() => (view = 'year')}>Year</button>
			<button aria-pressed={view === 'month'} onclick={() => (view = 'month')}>Month</button>
...
<div class="stage rise">
	{#if view === 'day'}
		<DayView date={cursor} />
	{:else if view === 'month'}
		<MonthView year={cursor.getFullYear()} month={cursor.getMonth()} onselect={openDay} />
	{:else}
